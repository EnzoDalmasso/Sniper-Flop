...
nivel2::nivel2(b2World* mundo)
{
	...
	act_sujetacaja = new actor(bdy_sujetacaja, fig_sujetacaja);
	...
	txt_nxtlevel->setString("Victoria");
	txt_nxtlevel->setPosition(50.5f, 94.f);
	...
	spr_quit->setPosition({ 51.8f, 96.f });
	spr_quit->setScale(0.01f, 0.01f);


}

void nivel2::set_camara(RenderWindow* ventana)
{
...
void nivel2::iniciar_fisica()
{
	...
	//suelo sujeta caja
	bdydef_sujetacaja.type = b2_staticBody;
	bdydef_sujetacaja.position = b2Vec2(55.5f, 93.f);

	bdy_sujetacaja = mundo1->CreateBody(&bdydef_sujetacaja);

	b2PolygonShape shp_piso_2;
	shp_piso_2.SetAsBox(1.f, 0.2f);

	fixdef_sujetacaja.shape = &shp_piso_2;
	fixdef_sujetacaja.restitution = 0.f;
	fixdef_sujetacaja.friction = 0.3f;
	fixdef_sujetacaja.density = 1.f;

	fix_sujetacaja = bdy_sujetacaja->CreateFixture(&fixdef_sujetacaja);

	//Caja
	bdydef_caja.type = b2_dynamicBody;
	bdydef_caja.position = b2Vec2(55.5f, 91.f);
	...
	bdydef_pendulo.position = b2Vec2(50.f, 92.f);

	bdy_pendulo = mundo1->CreateBody(&bdydef_pendulo);

	b2PolygonShape shp_pendulo;
	shp_pendulo.SetAsBox(0.1f, 0.8f);
...
void nivel2::iniciar_imagenes()
{
	...
	fig_sujetacaja->setFillColor(Color::White);
...
void nivel2::dibujar(RenderWindow* ventana)
{
	...
	if (estado_nivel == 0)
	{
		...
		act_sujetacaja->dibujar(ventana);
...
void nivel2::colision(RenderWindow *ventana)
{
	for (int i = 0; i < 6; i++)
	{
		rag_1->fig_rag[i];
...
void nivel2::dispararCannon(Vector2f mousecord)
{
...
void nivel2::moverCannon(Vector2f mousecord)
{
...
void nivel2::resortesPendulo(b2World& mundo)
{
	
	//Resortes Pendulo
	jntdef_pendulo.Initialize(bdy_pendulo, bdy_techo, b2Vec2(bdy_pendulo->GetPosition().x, bdy_pendulo->GetPosition().y + 0.06), b2Vec2(bdy_techo->GetPosition().x+ 2, bdy_techo->GetPosition().y));
	...
	bdy_pendulo->ApplyForce(b2Vec2(bdy_pendulo->GetPosition().x + 600, bdy_pendulo->GetPosition().y), bdy_pendulo->GetPosition(), false);

}

//Esta funcion recibe una posicion el cual detecta si el mouse colisiona con el sprite
bool nivel2::detectarBoton(Vector2f mouscord)
{
	//Si colisiona el mouse con el sprite 
	if (spr_quit->getGlobalBounds().contains(mouscord.x, mouscord.y))
	{
		return true;
	}
